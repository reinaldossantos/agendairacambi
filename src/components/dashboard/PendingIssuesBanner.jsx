...
export default function PendingIssuesBanner() {
  ...
  return <div role="alert" className="mb-5 flex flex-col gap-3 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-amber-950 shadow-sm dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100 sm:flex-row sm:items-center">
    ...
    <div className="flex-1"><strong className="block">Você possui {count} pendência{count === 1 ? "" : "s"}</strong><p className="text-sm">{vehicleIssues.length} de veículo, {activityIssues.length} de atividade e {bonusIssues.length} bonificação(ões) para autorizar. Os avisos permanecerão até a regularização.</p></div>
