...
const emptyForm = () => ({ title: "", request_type: "goods", urgency: "normal", needed_by: "", justification: "", management_project_id: "", edital_name: "", edital_number: "", edital_deadline: "", funding_source: "", program_ids: [], supplier_suggestion: "", delivery_location: "Iracambi", items: [emptyItem()] });
...
function presentationStatus(request) {
...
export default function PurchaseRequests() {
  ...
  function validate() {
    ...
    if (!form.program_ids.length) return "Informe ao menos um programa beneficiado diretamente.";
    return "";
  }
  async function saveDraft() {
    ...
    const payload = {
      ...
      beneficiary_person_ids: [], beneficiary_description: null,
  ...
  if (mode === "form") return <main className="mx-auto max-w-6xl space-y-5 p-4 md:p-8">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><button onClick={() => setMode("list")} className="text-sm font-bold text-primary">← Voltar</button><h1 className="mt-2 text-3xl font-black text-primary dark:text-white">{editingId ? "Editar solicitação" : "Nova solicitação de compra"}</h1><p className="text-on-surface-variant">Planeje a necessidade, o programa atendido e o investimento estimado.</p></div><div className="rounded-2xl bg-primary px-5 py-3 text-white"><span className="block text-xs uppercase opacity-75">Total estimado</span><strong className="text-xl">{money(total)}</strong></div></div>
    ...
    <Section title="2. Projeto, edital e fonte do recurso" icon="account_tree" tone="bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
      <ChoiceGroup title="Programa beneficiado diretamente *" options={programs} values={form.program_ids} onToggle={(id) => toggleArray("program_ids", id)} wide />
    ...
    <Section title="3. Itens solicitados" icon="inventory_2" tone="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
    ...
    {preview && <Preview form={form} total={total} programs={programs} projects={projects} busy={busy} onClose={() => setPreview(false)} onConfirm={submitRequest} />}
  </main>;
  ...
  return <main className="mx-auto max-w-screen-2xl space-y-6 p-4 md:p-8">
    ...
    <div className="grid gap-4 lg:grid-cols-2">{visible.map((request) => { const displayStatus = presentationStatus(request); return <article key={request.id} className="rounded-3xl border border-surface-variant bg-white p-5 shadow-sm transition hover:shadow-md dark:bg-gray-900">
      <div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><span className="material-symbols-outlined h-fit rounded-xl bg-amber-100 p-2 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">shopping_bag</span><div><p className="text-xs font-black uppercase tracking-wider text-on-surface-variant">Solicitação nº {String(request.request_number).padStart(5, "0")}</p><h2 className="mt-1 text-xl font-black text-primary dark:text-white">{request.title}</h2><p className="text-sm">{request.requester?.name}{request.project ? ` · ${request.project.title}` : ""}</p></div></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${displayStatus.tone}`}>{displayStatus.label}</span></div>
...
function ChoiceGroup({ title, options, values, onToggle, wide = false }) { return <div className={wide ? "md:col-span-2" : ""}><p className="mb-2 text-sm font-bold">{title}</p><div className="grid max-h-44 gap-1 overflow-auto rounded-xl border p-2 sm:grid-cols-2">{options.map((option) => <label key={option.id} className="flex cursor-pointer items-center gap-2 rounded-lg p-2 hover:bg-primary/5"><input type="checkbox" checked={values.includes(option.id)} onChange={() => onToggle(option.id)} className="rounded text-primary" /><span>{option.name}</span></label>)}</div></div>; }
...
function Details({ request, currentUser, approverIds, programs, projects, comment, setComment, busy, onClose, onEdit, onDelete, onDecide, onAdvance, onRecordStep }) {
  ...
  const displayStatus = presentationStatus(request);
  ...
  return <Modal title={`Solicitação nº ${String(request.request_number).padStart(5, "0")}`} onClose={onClose}><div className="space-y-5"><div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start"><div className="min-w-0"><h3 className="break-words text-2xl font-black leading-tight text-primary dark:text-white">{request.title}</h3><p className="mt-2">{request.requester?.name}</p></div><span className={`h-fit w-fit rounded-full px-3 py-1 text-sm font-bold sm:justify-self-end ${displayStatus.tone}`}>{displayStatus.label}</span></div><ApprovalProgress approvals={request.approvals} /><Summary label="Justificativa" value={request.justification} /><Summary label="Projeto" value={projects.find((item) => item.id === request.management_project_id)?.title || "Sem vínculo"} />{request.edital_name && <Summary label="Edital" value={`${request.edital_name}${request.edital_number ? ` · ${request.edital_number}` : ""}`} />}<Summary label="Programa beneficiado diretamente" value={<Names ids={request.program_ids} options={programs} />} /><Items items={request.items} /><div className="text-right text-xl font-black">Total estimado: {money(request.estimated_total)}</div>
    ...
    <div className="sticky bottom-0 -mx-5 -mb-5 mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-surface-variant bg-white/95 px-5 py-4 backdrop-blur dark:bg-gray-900/95 md:-mx-7 md:-mb-7 md:px-7"><div className="flex flex-wrap items-center gap-2">{(admin || (own && ["draft", "changes_requested"].includes(request.status))) && <button onClick={() => onEdit(request)} className="rounded-full border border-primary px-5 py-2 font-bold text-primary">Editar</button>}{admin && <button onClick={() => onDelete(request)} className="rounded-full px-4 py-2 font-bold text-red-600">Excluir</button>}{buyer && ["approved", "quotation", "ordered", "partially_received"].includes(request.status) && <button onClick={() => setConfirmingCancellation(true)} className="rounded-full border border-red-300 px-4 py-2 text-sm font-bold text-red-700">Cancelar solicitação definitivamente</button>}</div><button type="button" onClick={onClose} className="flex items-center gap-2 rounded-full bg-primary px-7 py-2.5 font-black text-white shadow"><span className="material-symbols-outlined text-[19px]">logout</span>Sair</button></div>
