...
export default function PurchaseRequests() {
  ...
  if (mode === "form") return <main className="mx-auto max-w-6xl space-y-5 p-4 md:p-8">
    ...
    <Section title="1. Necessidade" icon="shopping_cart">
    ...
    <Section title="2. Projeto, edital e fonte do recurso" icon="account_tree">
    ...
    <Section title="3. Quem será beneficiado" icon="groups">
    ...
    <Section title="4. Itens solicitados" icon="inventory_2">
  ...
  return <main className="mx-auto max-w-screen-2xl space-y-6 p-4 md:p-8">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-bold uppercase tracking-widest text-secondary">Aquisições</p><h1 className="text-3xl font-black text-primary dark:text-white">Solicitações de compras</h1><p className="text-on-surface-variant">Da necessidade à entrega, com aprovação e rastreabilidade.</p></div><button onClick={beginNew} className="rounded-full bg-[#ffd12f] px-6 py-3 font-black text-primary shadow-md">+ Nova solicitação</button></div>
    {message.text && <Alert message={message} />}
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric icon="pending_actions" label="Aguardando aprovação" value={stats.pending} /><Metric icon="verified" label="Aprovadas/em andamento" value={stats.approved} /><Metric icon="inventory" label="Recebidas" value={stats.received} /><Metric icon="payments" label="Volume estimado" value={money(stats.total)} /></div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">{visible.map((request) => <article key={request.id} className="rounded-3xl border border-surface-variant bg-white p-5 shadow-sm transition hover:shadow-md dark:bg-gray-900">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-on-surface-variant">Solicitação nº {String(request.request_number).padStart(5, "0")}</p><h2 className="mt-1 text-xl font-black text-primary dark:text-white">{request.title}</h2><p className="text-sm">{request.requester?.name}{request.project ? ` · ${request.project.title}` : ""}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${statusTone[request.status]}`}>{labels[request.status]}</span></div>
...
function Section({ title, icon, children }) { return <section className="rounded-3xl border border-surface-variant bg-white p-5 shadow-sm dark:bg-gray-900"><h2 className="mb-5 flex items-center gap-2 text-xl font-black text-primary dark:text-white"><span className="material-symbols-outlined">{icon}</span>{title}</h2><div className="grid gap-4 md:grid-cols-2">{children}</div></section>; }
...
function Metric({ icon, label, value }) { return <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-gray-900"><span className="material-symbols-outlined text-secondary">{icon}</span><strong className="mt-2 block text-2xl text-primary dark:text-white">{value}</strong><span className="text-sm text-on-surface-variant">{label}</span></div>; }
...
function Details({ request, currentUser, approverIds, programs, persons, projects, comment, setComment, busy, onClose, onEdit, onDelete, onDecide, onAdvance, onRecordStep }) {
  ...
  return <Modal title={`Solicitação nº ${String(request.request_number).padStart(5, "0")}`} onClose={onClose}><div className="space-y-4"><div className="flex flex-wrap justify-between gap-2"><div><h3 className="text-2xl font-black text-primary dark:text-white">{request.title}</h3><p>{request.requester?.name}</p></div><span className={`h-fit rounded-full px-3 py-1 text-sm font-bold ${statusTone[request.status]}`}>{labels[request.status]}</span></div><Summary label="Justificativa" value={request.justification} /><Summary label="Projeto" value={projects.find((item) => item.id === request.management_project_id)?.title || "Sem vínculo"} />{request.edital_name && <Summary label="Edital" value={`${request.edital_name}${request.edital_number ? ` · ${request.edital_number}` : ""}`} />}<Summary label="Programas beneficiados" value={<Names ids={request.program_ids} options={programs} />} /><Summary label="Pessoas beneficiadas" value={<Names ids={request.beneficiary_person_ids} options={persons} />} />{request.beneficiary_description && <Summary label="Outro público" value={request.beneficiary_description} />}<Items items={request.items} /><div className="text-right text-xl font-black">Total estimado: {money(request.estimated_total)}</div>
    ...
    <section className="rounded-2xl border border-surface-variant p-4"><h4 className="flex items-center gap-2 font-black text-primary dark:text-white"><span className="material-symbols-outlined">history</span>Linha do tempo completa</h4><p className="mt-1 text-xs text-on-surface-variant">Registros permanentes com autoria, data, valores, documentos e anexos.</p>
    ...
    <div className="flex flex-wrap justify-between gap-3 border-t pt-4"><div>{(admin || (own && ["draft", "changes_requested"].includes(request.status))) && <button onClick={() => onEdit(request)} className="rounded-full border border-primary px-5 py-2 font-bold text-primary">Editar</button>} {admin && <button onClick={() => onDelete(request)} className="ml-2 rounded-full px-4 py-2 font-bold text-red-600">Excluir</button>}</div>{buyer && ["approved", "quotation", "ordered", "partially_received"].includes(request.status) && <button onClick={() => onAdvance(request, "cancelled")} className="text-sm font-bold text-red-600">Cancelar solicitação</button>}</div>
  </div></Modal>;
}
function Modal({ title, onClose, children }) { return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-3" role="dialog" aria-modal="true"><div className="max-h-[94vh] w-full max-w-3xl overflow-auto rounded-3xl bg-white p-5 shadow-2xl dark:bg-gray-900 md:p-7"><div className="mb-4 flex justify-between"><h2 className="text-xl font-black text-primary dark:text-white">{title}</h2><button onClick={onClose} aria-label="Fechar"><span className="material-symbols-outlined">close</span></button></div>{children}</div></div>; }
