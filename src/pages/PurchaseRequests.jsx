...
function Details({ request, currentUser, approverIds, programs, projects, comment, setComment, busy, onClose, onEdit, onDelete, onDecide, onAdvance, onRecordStep }) {
  ...
  return <Modal title={`Solicitação nº ${String(request.request_number).padStart(5, "0")}`} onClose={onClose}><div className="space-y-4"><div className="flex flex-wrap justify-between gap-2"><div><h3 className="text-2xl font-black text-primary dark:text-white">{request.title}</h3><p>{request.requester?.name}</p></div><span className={`h-fit rounded-full px-3 py-1 text-sm font-bold ${statusTone[request.status]}`}>{labels[request.status]}</span></div><Summary label="Justificativa" value={request.justification} /><Summary label="Projeto" value={projects.find((item) => item.id === request.management_project_id)?.title || "Sem vínculo"} />{request.edital_name && <Summary label="Edital" value={`${request.edital_name}${request.edital_number ? ` · ${request.edital_number}` : ""}`} />}<Summary label="Programa beneficiado diretamente" value={<Names ids={request.program_ids} options={programs} />} /><Items items={request.items} /><div className="text-right text-xl font-black">Total estimado: {money(request.estimated_total)}</div>
    ...
    <div className="sticky bottom-0 -mx-5 -mb-5 mt-5 flex justify-end border-t border-surface-variant bg-white/95 px-5 py-4 backdrop-blur dark:bg-gray-900/95 md:-mx-7 md:-mb-7 md:px-7"><button type="button" onClick={onClose} className="flex items-center gap-2 rounded-full bg-primary px-7 py-2.5 font-black text-white shadow"><span className="material-symbols-outlined text-[19px]">logout</span>Sair</button></div>
