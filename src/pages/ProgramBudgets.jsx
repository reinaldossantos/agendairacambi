...
export default function ProgramBudgets() {
  ...
  const loadData = useCallback(async () => {
    const [programResult, budgetResult, reportResult, purchaseResult, managerResult] = await Promise.all([
      ...
      supabase.from("program_budgets").select("*, lines:program_budget_lines(*)").order("fiscal_year", { ascending: false }),
  ...
  const totals = useMemo(() => yearBudgets.reduce((result, budget) => {
  ...
  async function saveBudget(event) {
    ...
    setSaving(true);
    const header = { program_id: form.program_id, fiscal_year: Number(form.fiscal_year), name: form.name.trim(), description: form.description.trim() || null, notes: form.notes.trim() || null, created_by: currentUser.id };
    const headerResult = editingId
      ? await supabase.from("program_budgets").update(header).eq("id", editingId).select().single()
      : await supabase.from("program_budgets").insert(header).select().single();
  ...
  return <main className="mx-auto max-w-screen-2xl space-y-6 p-4 md:p-8">
    ...
    <section className="grid gap-5 xl:grid-cols-2">{yearBudgets.map((budget) => <BudgetCard key={budget.id} budget={budget} program={programs.find((program) => program.id === budget.program_id)} usage={programUsage(budget.program_id, Number(year), reports, purchases)} manageable={canManage(budget.program_id)} onEdit={() => beginEdit(budget)} onDelete={() => setDeleteTarget(budget)} />)}</section>
...
function BudgetCard({ budget, program, usage, manageable, onEdit, onDelete }) {
