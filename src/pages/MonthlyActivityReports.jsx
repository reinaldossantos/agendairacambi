...
export default function MonthlyActivityReports() {
  ...
  async function regenerateReport() {
    ...
    const { data, error } = await supabase.from("monthly_activity_reports").update({
      activity_snapshot: activities, indicators, team_names: teamNames,
...
function MonthYearPicker({ value, onChange }) {
  ...
  return <div className="flex items-center gap-2 rounded-xl border border-surface-variant bg-surface p-1.5 dark:border-gray-700 dark:bg-gray-800">
    ...
    <select aria-label="Mês" value={month} onChange={(event) => onChange(`${year}-${event.target.value}`)} className="min-w-0 flex-1 rounded-lg border-0 bg-white py-2 pl-2 pr-7 capitalize text-sm font-bold text-primary focus:ring-primary dark:bg-gray-700 dark:text-white">{monthOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
    <select aria-label="Ano" value={year} onChange={(event) => onChange(`${event.target.value}-${month}`)} className="w-24 rounded-lg border-0 bg-white py-2 pl-2 pr-7 text-sm font-bold text-primary focus:ring-primary dark:bg-gray-700 dark:text-white">{years.map((option) => <option key={option}>{option}</option>)}</select>
