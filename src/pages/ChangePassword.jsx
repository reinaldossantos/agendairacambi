...
export default function ChangePassword() {
  ...
  async function submit(event) { event.preventDefault(); if (password.length < 8) return setMessage("A nova senha deve ter pelo menos 8 caracteres."); if (password !== confirmation) return setMessage("As senhas não coincidem."); setSaving(true); const { error } = await supabase.auth.updateUser({ password }); const profileResult = error ? null : await supabase.rpc("complete_password_change"); setSaving(false); if (error || profileResult?.error) setMessage(error?.message || profileResult.error.message); else await refreshProfile(); }
