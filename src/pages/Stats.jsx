...
export default function Stats() {
  ...
  const handleStatusClick = (status) => {
    navigate(`/history?status=${encodeURIComponent(status)}`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-0">
      ...
      {/* Cards de status - agora clicáveis */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {["Planejado", "Em andamento", "Realizado", "Pendente"].map((st) => {
          ...
          const colorMap = {
            ...
            "Em andamento":
              "bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/50 cursor-pointer transition-colors",
            Realizado:
              "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/50 cursor-pointer transition-colors",
            Pendente:
              "bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 cursor-pointer transition-colors",
          };
          return (
            <div
              key={st}
