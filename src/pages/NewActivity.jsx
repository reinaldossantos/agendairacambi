  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/55 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="vehicle-booking-title">
    <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl dark:bg-gray-900 sm:rounded-3xl">
      ...
      <div className="space-y-4 p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-bold text-primary dark:text-white">Veículo<select required className="mt-1 w-full rounded-xl border border-surface-variant bg-surface px-3 py-2.5 dark:bg-gray-800" value={value.vehicle_id} onChange={(event) => { const capacity = vehicles.find((vehicle) => vehicle.id === event.target.value)?.capacity; onChange({ ...value, vehicle_id: event.target.value, passenger_ids: capacity ? value.passenger_ids.slice(0, capacity) : value.passenger_ids }); }}><option value="">Selecione</option>{vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.name} · {vehicle.plate} ({vehicle.capacity} lugares)</option>)}</select></label>
        ...
        <TeamMemberSelector people={people} selectedIds={value.passenger_ids} onChange={(passenger_ids) => onChange({ ...value, passenger_ids })} label="Quem estará no veículo?" maxSelected={selectedVehicle?.capacity || null} />
